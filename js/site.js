...
(function(){
  document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      // CONTACT FORM: add client-side submit, honeypot check and inline status message with spinner
    ...
    (function(){
      ...
      function applyConsent(value) {
        ...
        if (typeof gtag === 'function') {
          ...
          // Only configure GA after consent granted
          if (granted) {
      ...
      function createBanner() {
        ...
        banner.style.cssText = [
          ...
          'border:1px solid rgba(0,0,0,0.1)','box-shadow:0 12px 32px rgba(0,0,0,0.18)','border-radius:12px',
          'padding:1rem','max-width:720px','margin:0 auto','font:14px/1.5 system-ui, -apple-system, Segoe UI, Roboto, Arial'
        ].join(';');
        ...
        // Focus accessibility
        (btnAccept || banner).focus();
      }

      // On load: respect existing choice or show banner
      const choice = getCookie(CONSENT_COOKIE);
      if (choice === 'granted' || choice === 'denied') {
        applyConsent(choice);
      } else {
        // default is already denied from inline tag; show banner
        createBanner();
      }
    })();
  });
